...
def plot_boxes(image, model, confidence_threshold):
    ...
    results = model(image)
    for _, (xmin, ymin, xmax, ymax, confidence, class_idx, class_name) in (
        results.pandas().xyxy[0].iterrows()
    ):
...
@click.option("--confidence-threshold", type=click.FloatRange(0, 0.99), default=0.5)
@click.option(
    "--url", type=click.STRING, default="https://www.youtube.com/watch?v=sM-rZWOyJzc"
)
def youtube(model_type, confidence_threshold, url):
    ...
    model = torch.hub.load(
        "ultralytics/yolov5", f"yolov5{model_type}", pretrained=True, device=DEVICE
    )
